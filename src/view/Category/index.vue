    ...
    categoryFilte() {
      if (this.query) {
        return this.category.filter((category) => {
          return category.CategoryName.toLowerCase().indexOf(this.query.toLowerCase()) != -1;
        });
      } else {
  ...
  methods: {
    ...
    onDelete(item) {
      if (confirm("Bạn có chắc muốn xóa danh mục " + item.CategoryName)) {
          CategoryServices.delete(item._id)
            .then((response) => {
            this.category.splice(this.category.findIndex(e => e._id == item._id), 1).push(response.data);
