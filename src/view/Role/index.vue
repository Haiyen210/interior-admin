            ...
            <div class="layout-px-spacing" style="margin-top: 17px;">
                ...
                <div class="row layout-top-spacing">
                    <div id="tableProgress" class="col-lg-12 col-12 layout-spacing">
                        ...
                        <div class="statbox widget box box-shadow"
                            ...
                            <div class="row"  style="margin-top: 26px;">
                                <div class="col-12 col-sm-5 d-flex justify-content-sm-start justify-content-center" >
                                    <a type="submit" class="btn btn-success mt-3" v-on:click.prevent="onAdd" style="margin-left: 24px;">Thêm mới<svg
                        ...
                        <RoleEdit :role="showEdit" v-if="isShowEdit == true" @ShowEditData="getEdit($event)" />
                        <RoleAdd v-if="isShowAdd == true" @ShowData="getData($event)" />
                    </div>
                </div>
            </div>
        </div>
  ...
  computed: {
    ...
    categoryFilte() {
      if (this.query) {
        return this.role.filter((role) => {
          return role.Name.toLowerCase().indexOf(this.query.toLowerCase()) != -1;
        });
      } else {
        return this.role;
      }
    ...
    paginated() {
      if (this.resultCount > 5) {
        return this.role.slice(this.indexStart, this.indexEnd, this.totalPaginate);
      } else {
        return this.role;
      }
    },
  ...
  methods: {
    ...
    onDelete(item) {
      if (confirm("Bạn có chắc muốn xóa danh mục " + item.Name)) {
          RoleServices.delete(item._id)
            .then((response) => {
            this.role.splice(this.role.findIndex(e => e._id == item._id), 1).push(response.data);
